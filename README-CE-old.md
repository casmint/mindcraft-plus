...
The default `runtime_mode` is `"direct"`, which preserves the existing command parser, `ActionManager`, skills, and reactive modes. Set `runtime_mode` in `settings.js`, or set `runtime.mode` in a profile, to `"actiongraph"` only when testing registered ActionGraph tasks. ActionGraph v0 currently exposes `!basicStartup`; unsupported graph tasks return a clear fallback message and direct commands continue to work unchanged.
